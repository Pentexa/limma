...
pub async fn crawl(
    ...
    queue.push_back(QueueItem { url: start_url.to_string(), depth: 0 });
    
    let mut main_final_url = start_url.to_string();
    let mut main_chain = Vec::new();
    
    emit_event("SCAN_STARTED", "INFO", format!("Initiating Phase 4 Crawl starting at {}", start_url), None);

    while let Some(item) = queue.pop_front() {
        if pages.len() as u32 >= max_pages {
            emit_event("CRAWL_LIMIT_REACHED", "WARN", "Hit crawler page limit constraint".to_string(), None);
            break;
        }
        ...
        match fetch_res {
            Ok((final_url, status, headers, body, latency, chain)) => {
                ...
                emit_event("PAGE_CRAWLED", "INFO", format!("Successfully fetched {}", item.url), Some(serde_json::to_value(&sp).unwrap_or(serde_json::Value::Null)));

                if !detected.is_empty() {
                    emit_event("TECH_DETECTED", "INFO", format!("Fingerprinted {} technologies on {}", detected.len(), item.url), Some(serde_json::to_value(&detected).unwrap_or(serde_json::Value::Null)));
                }
                ...
                if item.depth < 2 && ct.unwrap_or_default().contains("text/html") {
                    ...
                    for link in extracted {
                        if !visited.contains(&link) {
                            ...
                            if link_l.contains("login") || link_l.contains("about") || link_l.contains("contact") || link_l.contains("dashboard") || link_l.contains("admin") {
            ...
            Err(e) => {
                emit_event("FETCH_ERROR", "ERROR", format!("Failed to fetch {}: {}", item.url, e), None);
            }
        }
    }

    emit_event("CRAWL_COMPLETE", "INFO", format!("Crawl complete. Scanned {} pages.", pages.len()), None);

    Ok(CrawlResult { pages, events, final_url: main_final_url, main_chain })
}
...
async fn fetch_manual_redirects(
    client: &Client,
    start_url: &str,
) -> Result<(String, u16, HashMap<String, String>, String, u64, Vec<RedirectChainEntry>), String> {
    ...
    loop {
        ...
        let resp = match resp_result {
            Ok(r) => r,
            Err(e) => {
                redirect_chain.push(RedirectChainEntry { url: current_url.clone(), status_code: 0 });
        ...
        redirect_chain.push(RedirectChainEntry { url: current_url.clone(), status_code });
        ...
        if status.is_redirection() {
            ...
            for (name, value) in resp.headers() {
                headers_map.insert(name.as_str().to_lowercase(), value.to_str().unwrap_or("").to_string());
            }
            let body = resp.text().await.unwrap_or_default();
            let latency_ms = total_start.elapsed().as_millis() as u64;
            
            return Ok((current_url, status_code, headers_map, body, latency_ms, redirect_chain));
        }
    }
}
