use super::normalizer::{
    ApiDiscovererNormalizer, FindingNormalizer, ServerInvestigatorNormalizer, WebScannerNormalizer,
};
...
impl SecurityAuditorRepository for HttpSecurityAuditor {
    async fn audit(&self, url_str: &str) -> Result<SecurityReport, String> {
        ...
        let essential_headers = vec![
            ...
            (
                ...
                "HSTS forces encrypted connections (HTTPS).",
                15,
            ),
            (
                "x-frame-options",
            ...
            (
                "referrer-policy",
                "Controls leakage of referrer information to external sites.",
                5,
            ),
            (
                "permissions-policy",
        ...
        if let Some(cookie_header) = headers.get("set-cookie") {
            ...
            if !cookie_str.contains("samesite") {
                ...
                recommendations.push(
                    "Cookies do not have a 'SameSite' attribute, increasing CSRF risk.".to_string(),
                );
            }
        }
            ...
            if let Ok(robot_resp) = self.client.get(&robot_url).send().await {
                if robot_resp.status().is_success() {
                    ...
                    let suspicious_keywords =
                        vec!["admin", "login", "wp-", "config", ".env", "backup", "db"];
    ...
    async fn normalize_all(
        ...
        log.push(format!(
            "[Normalizer] Started normalization for target: {}",
            target
        ));
        ...
        log.push(format!(
            "[Normalizer] Extracted {} findings from Web Scanner.",
            web_findings.len()
        ));
        ...
        log.push(format!(
            "[Normalizer] Extracted {} findings from Server Investigator.",
            server_findings.len()
        ));
        ...
        log.push(format!(
            "[Normalizer] Extracted {} findings from API Discoverer.",
            api_findings.len()
        ));
        ...
        for mut f in all_findings {
                ...
                && f.confidence == ConfidenceLevel::Low
            {
                is_fp = true;
                fp_reason =
                    "Suspicious endpoint match was too generic (Low confidence)".to_string();
            }
            ...
            if is_fp {
                ...
                tracing::debug!(
                    "FP Filter triggered -> Dropped finding: {} | Reason: {}",
                    f.summary,
                    fp_reason
                );
            } else {
        ...
        log.push(format!(
            "[Normalizer] FP Mitigation Complete. Filtered out {} false positive anomalies.",
            rejected
        ));
        log.push(format!(
            "[Normalizer] Proceeding with {} verified findings.",
            accepted
        ));
        ...
        let matched_count = rule_results
            .iter()
            .filter(|r| r.outcome == RuleOutcome::Matched)
            .count();
        let partially_matched_count = rule_results
            .iter()
            .filter(|r| r.outcome == RuleOutcome::PartiallyMatched)
            .count();

        log.push(format!(
            "[RuleEngine] Evaluated {} findings against {} rules.",
            all_findings.len(),
            engine.rules.len()
        ));
        log.push(format!(
            "[RuleEngine] Matched: {} rules. Partially Matched: {} rules.",
            matched_count, partially_matched_count
        ));
        ...
        log.push(format!(
            "[Correlator] Analyzed {} findings, produced {} correlation groups.",
            all_findings.len(),
            correlations.len()
        ));
        ...
        log.push(
            "[Correlator] Mapping multi-signal clusters back into isolated findings...".to_string(),
        );
            ...
            for link in &corr.linked_findings {
                if let Some(f) = all_findings.iter_mut().find(|x| x.id == link.finding_id) {
                    if f.correlation_group_id.is_none() {
                        ...
                        f.related_findings =
                            siblings.iter().filter(|s| s.id != f.id).cloned().collect();
        ...
        log.push(format!(
            "[Correlator] Rendered {} findings into insight clusters. Isolated findings: {}.",
            correlated_finding_count,
            all_findings.len().saturating_sub(correlated_finding_count)
        ));
        ...
        log.push(format!(
            "[Scorer] Scored {} findings. Boosted: {}, Downgraded: {}, Overall Risk: {:.1}",
            scoring_stats.total_scored,
            scoring_stats.boosted,
            scoring_stats.downgraded,
            scoring_stats.overall_risk_score
        ));
        ...
        log.push(format!(
            "[Canonicalizer] Condensed {} raw findings into {} unique canonical findings.",
            all_findings.len(),
        ...
        exploitability_engine
            .evaluate(target, &mut canonical_findings)
            .await;
        log.push("[ExploitabilityEngine] Runtime assessments complete.".to_string());

        // --- Phase 8: Attack Path Correlator ---
        log.push(
            "[AttackPathEngine] Analyzing contextual relationships to detect exploit scenarios..."
                .to_string(),
        );
        ...
        log.push(format!(
            "[AttackPathEngine] Derived {} potential high-level attack paths.",
            attack_paths.len()
        ));
        ...
        autonomous_engine
            .verify_all(target, &mut canonical_findings, &mut attack_paths)
            .await;
        log.push(
            "[AutonomousVerificationEngine] Validation state machine concluded successfully."
                .to_string(),
        );
        ...
        let pool = self
            .pool
            .clone()
            .expect("Database pool not initialized for HttpSecurityAuditor");
        ...
        let mut calibration_engine =
            super::confidence_calibration::ConfidenceCalibrationEngine::new(pool.clone());
        calibration_engine
            .update_from_scan(&mut canonical_findings, &learning_engine)
            .await;
        ...
        prioritization_engine
            .evaluate_all(&mut canonical_findings, &mut attack_paths, &learning_engine)
            .await;
        log.push(format!(
            "[ThreatPrioritizationEngine] Successfully ranked {} threats and {} attack paths.",
            canonical_findings.len(),
            attack_paths.len()
        ));
        ...
        let audit_certainty = if all_findings.iter().any(|f| f.correlation_count > 0) {
            Some(CertaintyNote {
                ...
                reason:
                    "Bulgular çoklu modül tarafından doğrulandı ve korelasyon grupları oluşturuldu"
                        .to_string(),
            })
        } else if !all_findings.is_empty() {
            Some(CertaintyNote {
                level: CertaintyLevel::Likely,
                reason: "Bulgular tespit edildi ama çapraz modül doğrulaması yapılamadı"
                    .to_string(),
            })
        } else {
            Some(CertaintyNote {
                level: CertaintyLevel::Unknown,
                reason: "Hiçbir güvenlik bulgusu üretilemedi — sonuçlar güvenilir değil"
                    .to_string(),
            })
        };
