...
impl EndpointClassifier {
    ...
    pub fn assess_auth(content: &str, path: &str) -> f32 {
        ...
        if content_lower.contains("authorization") {
            prob += 0.40;
        }
        if content_lower.contains("bearer ") {
            prob += 0.30;
        }
        if content_lower.contains("jwt") {
            prob += 0.20;
        }
        if content_lower.contains("x-api-key") {
            prob += 0.35;
        }
        if content_lower.contains("credentials") {
            prob += 0.25;
        }
        if content_lower.contains("session") {
            prob += 0.15;
        }
