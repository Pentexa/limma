...
fn interpret_banner(banner: &str) -> String {
    if banner.starts_with("SSH-") {
        return format!("SSH banner detected: {}", banner.chars().take(40).collect::<String>());
    }
    if banner.starts_with("220") && (banner.contains("FTP") || banner.contains("ftp")) {
        return "FTP server greeting (220)".to_string();
    }
    if banner.starts_with("220") && (banner.contains("SMTP") || banner.contains("smtp") || banner.contains("mail")) {
        return "SMTP server greeting (220)".to_string();
    }
    if banner.starts_with("220") {
        return format!("Service greeting (220): {}", banner.chars().take(60).collect::<String>());
    }
    ...
    format!("Unrecognized banner: {}", banner.chars().take(50).collect::<String>())
}
...
pub fn service_hint_from_banner(evidence: &ProbeEvidence) -> Option<&'static str> {
    ...
    if interp.contains("SSH") { return Some("SSH"); }
    if interp.contains("FTP") { return Some("FTP"); }
    if interp.contains("SMTP") { return Some("SMTP"); }
    if interp.contains("POP3") { return Some("POP3"); }
    if interp.contains("IMAP") { return Some("IMAP"); }
    if interp.contains("HTTP") { return Some("HTTP"); }
    if interp.contains("Redis") { return Some("Redis"); }
    None
}
