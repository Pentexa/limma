...
pub async fn probe_with_fallback(
    ...
    let banner_timeout = Duration::from_millis(timeout_ms / 2);
    let tls_timeout = Duration::from_millis(timeout_ms);
    let http_timeout = Duration::from_millis(timeout_ms);
    let greeting_timeout = Duration::from_millis(timeout_ms / 2);
    ...
    match strategy {
        ProbeStrategy::HttpFirst => {
                ...
                http_probe::probe_http_plain(host, &mut stream, http_timeout).await
            {
                ...
                if let Some(mut new_stream) = try_connect(&addr, connect_timeout).await {
                    if let Some(ev) =
        ...
        ProbeStrategy::TlsFirst => {
            ...
            if let Some((ts, ev)) = tls_probe::probe_tls(host, stream, tls_timeout).await {
                ...
                if let Some(new_stream) = try_connect(&addr, connect_timeout).await {
                    if let Some((ts2, _)) =
                ...
                if let Some(mut new_stream) = try_connect(&addr, connect_timeout).await {
                        ...
                        http_probe::probe_http_plain(host, &mut new_stream, http_timeout).await
                    {
                        ...
                        if let Some(mut banner_stream) = try_connect(&addr, connect_timeout).await {
                            if let Some(ev) =
...
async fn try_connect(addr: &str, connect_timeout: Duration) -> Option<TcpStream> {
