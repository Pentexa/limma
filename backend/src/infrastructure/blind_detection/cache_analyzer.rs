...
impl CacheAnalyzer {
    ...
    pub async fn detect_cache_deception(
        ...
        // We simulate having a session by sending a dummy session cookie.
        // In a real scenario, this would use the active session cookie provided by the scanner.
        let dummy_session = "sessionid=test_auth_user_12345";
        ...
        for ext in extensions {
            ...
            let auth_res = self.client.get(&test_url)
                .header(COOKIE, dummy_session)
                .send()
                .await;

            if let Ok(auth_response) = auth_res {
                let cache_status = auth_response.headers()
                    .get("X-Cache")
                ...
                if let Ok(auth_body) = auth_response.text().await {
                    if (cache_status.contains("HIT") || cache_status.contains("MISS")) && auth_body.to_lowercase().contains("<html") {
                        
                        // 2. Unauthenticated Request
                        let unauth_res = self.client.get(&test_url)
                            .send()
                            .await;
                                ...
                                if unauth_body == auth_body && unauth_body.len() > 100 {
                                    findings.push(RawBlindFinding {
                                        ...
                                        evidence: BlindEvidence {
                                            dom_snapshot: Some(unauth_body[..std::cmp::min(500, unauth_body.len())].to_string()),
