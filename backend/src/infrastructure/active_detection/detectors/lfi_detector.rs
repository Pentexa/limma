...
impl VulnDetector for LfiDetector {
    fn supported_types(&self) -> Vec<ActiveVulnType> {
        vec![ActiveVulnType::LocalFileInclusion, ActiveVulnType::RemoteFileInclusion, ActiveVulnType::PathTraversal]
    }
            ...
            if let Some(file) = Self::check_file_content(&body) {
                findings.push(ActiveVulnFinding {
                    ...
                    evidence: ActiveVulnEvidence {
                        ...
                        additional_notes: vec![payload_def.description.clone(), format!("Sensitive file read: {}", file)],
                    },
