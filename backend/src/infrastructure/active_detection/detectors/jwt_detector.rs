...
impl VulnDetector for JwtDetector {
    fn supported_types(&self) -> Vec<ActiveVulnType> {
        vec![ActiveVulnType::JwtNoneAlgorithm, ActiveVulnType::JwtWeakSecret]
    }
    ...
    async fn detect(&self, target_url: &str, _parameter: &str, scan_id: Uuid, payload_selector: &crate::infrastructure::active_detection::payload_selector::PayloadSelector, rate_limit_ms: u64, waf_monitor: std::sync::Arc<crate::infrastructure::safety::waf_monitor::WafMonitor>, _baseline: Option<&crate::infrastructure::active_detection::differential::BaselineProfile>) -> Result<Vec<ActiveVulnFinding>, String> {
        ...
        for payload_def in &none_payloads {
            ...
            let mut req = self.client.get(target_url).header("Authorization", format!("Bearer {}", payload_def.payload));
            ...
            if status == 200 && !body.contains("invalid") && !body.contains("unauthorized") {
                findings.push(ActiveVulnFinding {
                    ...
                    target_url: target_url.to_string(), affected_parameter: "Authorization header".to_string(),
                    http_method: "GET".to_string(), payload_used: payload_def.payload.clone(),
                    evidence: ActiveVulnEvidence {
                        request_raw: format!("GET {} HTTP/1.1\nAuthorization: Bearer {}", target_url, payload_def.payload),
                        ...
                        additional_notes: vec![payload_def.description.clone(),
                            "Server accepted JWT with 'none' algorithm — critical auth bypass".to_string()],
                    },
