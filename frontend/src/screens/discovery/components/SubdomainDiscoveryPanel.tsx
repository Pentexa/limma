...
import type { ApiSubdomainDiscoveryResult, ApiSubdomainAsset } from "@/shared/types/api";
import {
  Globe, Shield, AlertTriangle, CheckCircle, Activity,
  Clock, Filter, Search, ChevronDown, ChevronRight,
  Wifi, WifiOff, ExternalLink, Server, Tag, Fingerprint,
  BarChart3, Zap, Eye,
} from "lucide-react";

interface SubdomainDiscoveryPanelProps {
  data: ApiSubdomainDiscoveryResult;
}
...
export function SubdomainDiscoveryPanel({ data }: SubdomainDiscoveryPanelProps) {
  ...
  const durationSec = (metrics.scan_duration_ms / 1000).toFixed(1);
    ...
    <div className="space-y-4 w-full">
      ...
      <div className="flex flex-wrap items-center gap-3 p-3 bg-[#080808] rounded-md border border-border/30 shadow-inner">
        ...
        <div className="flex items-baseline gap-1.5 bg-[#0a0a0a] border border-border/20 px-2.5 py-1 rounded ml-auto">
          ...
          <span className="font-mono text-[11px] text-foreground font-bold">{data.domain}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2">
        {[
          { label: "Candidates", value: metrics.total_candidates, icon: Search, color: "text-blue-400" },
          { label: "Validated", value: metrics.validated_count, icon: CheckCircle, color: "text-emerald-400" },
          { label: "HTTP Alive", value: metrics.http_alive_count, icon: Wifi, color: "text-cyan-400" },
          { label: "Wildcard Filtered", value: metrics.wildcard_filtered_count, icon: Filter, color: "text-orange-400" },
          { label: "Passive Sources", value: metrics.passive_source_count, icon: Eye, color: "text-violet-400" },
          { label: "Active Sources", value: metrics.active_source_count, icon: Zap, color: "text-yellow-400" },
      ...
      {/* ── Wildcard DNS Alert ── */}
      {data.wildcard_dns.is_wildcard && (
        <div className="flex items-start gap-3 p-3.5 bg-orange-500/[0.04] border border-orange-500/20 rounded-md">
          <AlertTriangle className="h-4 w-4 text-orange-400 shrink-0 mt-0.5 drop-shadow-[0_0_4px_#f97316]" />
          <div className="space-y-1">
            <span className="text-[12px] font-bold text-orange-400">Wildcard DNS Detected</span>
            <p className="text-[11px] font-mono text-muted-foreground/70">
              Test: <span className="text-foreground/80">{data.wildcard_dns.test_subdomain}</span>
              {data.wildcard_dns.resolved_ips.length > 0 && (
                <> → IPs: <span className="text-foreground/80">{data.wildcard_dns.resolved_ips.join(", ")}</span></>
              )}
            </p>
          </div>
        </div>
      ...
      {/* ── Certainty Badge ── */}
      {data.discovery_certainty && (
        <div className={cn(
          "flex items-center gap-2.5 px-3.5 py-2 rounded-md border text-[11px] font-medium",
          data.discovery_certainty.level === "certain"
            ? "bg-emerald-500/[0.04] border-emerald-500/20 text-emerald-400"
            : data.discovery_certainty.level === "probable"
            ? "bg-blue-500/[0.04] border-blue-500/20 text-blue-400"
            : "bg-yellow-500/[0.04] border-yellow-500/20 text-yellow-400"
        )}>
          <Shield className="h-3.5 w-3.5 shrink-0" />
          <span className="font-bold uppercase tracking-wider text-[10px]">{data.discovery_certainty.level}</span>
          <span className="text-muted-foreground/70">{data.discovery_certainty.reason}</span>
        </div>
      )}
      ...
      <div className="text-[10px] font-mono text-muted-foreground/30 text-right">
        Scan completed at {new Date(data.scan_timestamp).toLocaleString()}
      </div>
    </div>
  );
}
